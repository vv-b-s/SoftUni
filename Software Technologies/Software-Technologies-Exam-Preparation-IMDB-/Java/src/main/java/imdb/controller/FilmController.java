...
public class FilmController {
	...
	public String createProcess(FilmBindingModel filmBindingModel) {
		Film film = new Film(filmBindingModel.getName(), filmBindingModel.getGenre(),filmBindingModel.getDirector(),
				filmBindingModel.getYear());
		if(film.allDataHasValue()) {
	...
	public String editProcess(@PathVariable int id, FilmBindingModel filmBindingModel) {
		Film film = filmRepository.findOne(id);
		Film newData = new Film(filmBindingModel.getName(), filmBindingModel.getGenre(),filmBindingModel.getDirector(),
				filmBindingModel.getYear());
		film.mergeData(newData);

		if(film.allDataHasValue()){
			filmRepository.saveAndFlush(film);
			return "redirect:/";
		}
		return String.format("redirect:/edit/%d",film.getId());
